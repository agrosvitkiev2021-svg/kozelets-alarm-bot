...
def get_neptun_threats():
    ...
    try:
        ...
        if response.status_code != 200:
            print("Помилка API:", response.text[:500])
            return []
        ...
        print(f"Отримано загроз: {len(threats)}")
        return threats
...
def build_threat_message(threat):
    ...
    if locality:
        ...
        display_place = str(place)
    elif district:
        display_place = str(district)
    elif region:
        display_place = str(region)
    else:
    ...
    source_count = get_value(threat, "sourceCount")
    ...
    message = []
    message.append("🛰 <b>ПОВІТРЯНА ЗАГРОЗА</b>")
    message.append("")
    message.append(f"⚠️ <b>Тип:</b> {html.escape(str(threat_type))}")
    message.append(f"📍 <b>Район:</b> {html.escape(display_place)}")
    ...
    message.append("")
    message.append(f'🔗 <a href="{NEPTUN_URL}">Дані про загрозу</a>')
...
def process_threats(state):
    ...
    for threat in threats:
        ...
        if not is_active_threat(threat):
            continue
    ...
    print(f"Активних загроз поблизу: {len(current)}")
    print(f"Нових загроз: {len(new_ids)}")

    for threat_id in new_ids:
        message = build_threat_message(current[threat_id])
    ...
    if current and not new_ids and now - last_update >= THREAT_UPDATE_SECONDS:
        print("Оновлюю активні загрози.")
        for threat_id, threat in list(current.items())[:5]:
            message = build_threat_message(threat)
            telegram_send(message)
    ...
    if previous and not current:
        telegram_send(
            ...
            "Активних загроз поблизу контрольованих населених пунктів "
            "не виявлено."
        )
        state["last_threat_update"] = now
...
def make_news_id(title, link):
    raw = str(title).strip() + "|" + str(link).strip()
    ...
    for source_name, feed_url in NEWS_FEEDS:
        try:
            ...
            for entry in feed.entries:
                ...
                published_time = parse_news_date(entry)
                if published_time is None:
                    continue

                age = now - published_time
                if age < 0 or age > NEWS_MAX_AGE_MINUTES * 60:
                    continue

                item_id = make_news_id(title, link)
                ...
                message = (
                    ...
                    f"📍 <b>{safe_source}</b>\n"
                    f"{safe_title}\n\n"
                    f'<a href="{safe_link}">🔗 Читати новину</a>'
                )
            ...
            print(f"Помилка новин {source_name}: {e}")

    state["published_news"] = published[-500:]
    print(f"Нових новин опубліковано: {new_count}")
...
def process_weather():
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"Open-Meteo помилка: {response.status_code}")
            return
...
def process_dashboard(state):
    ...
    if active_count:
        status = "🔴 Є активна загроза"
    else:
        status = "🟢 Активної загрози немає"

    message = (
        "📊 <b>СТАН КАНАЛУ</b>\n\n"
        f"🕐 Оновлено: {current_time_string()}\n"
        f"🚨 Активних загроз: {active_count}\n"
        f"{status}\n\n"
        "📍 Моніторинг:\n"
        "• Козелець\n"
        "• Остер\n"
...
def process_promo():
    ...
    message = (
        ...
        "👉 <b>Підписуйтесь та діліться каналом з близькими.</b>"
    )
...
def process_history():
    ...
    message = (
        ...
        "🏛️ Місто має багату історію, архітектурну спадщину та цікаві місця, про які варто пам'ятати.\n\n"
...
def main():
    print("")
    print("ЗАПУСК БЕЗПЕРЕРВНОГО ЦИКЛУ БОТА")
    print("Перевірка загроз — кожні 5 хвилин")
    print("Новини — кожні 30 хвилин")
    ...
    while True:
        try:
            ...
            # 1. ЗАГРОЗИ (Кожні 5 хвилин)
            try:
                process_threats(state)
            except Exception as e:
                print(f"Критична помилка загроз: {e}")

            # 2. НОВИНИ (Кожні 30 хвилин)
            ...
            if now - last_news >= NEWS_CHECK_SECONDS:
                try:
                    ...
                    print(f"Критична помилка новин: {e}")

            # 3. ПОГОДА (Кожні 10 хвилин)
            ...
            if now - last_weather >= WEATHER_CHECK_SECONDS:
                try:
                    ...
                    print(f"Критична помилка погоди: {e}")

            # 4. ПАНЕЛЬ (Кожні 10 хвилин)
            last_dashboard = state.get("last_dashboard_check", 0)
            if now - last_dashboard >= DASHBOARD_CHECK_SECONDS:
                try:
                    process_dashboard(state)
                    state["last_dashboard_check"] = now
                except Exception as e:
                    print(f"Критична помилка панелі: {e}")

            # 5. ПРОМО (Кожні 6 годин)
            last_promo = state.get("last_promo_check", 0)
            if now - last_promo >= PROMO_CHECK_SECONDS:
                try:
                    process_promo()
                    state["last_promo_check"] = now
                except Exception as e:
                    print(f"Критична помилка промо: {e}")

            # 6. ІСТОРІЯ (Кожні 12 годин)
            last_history = state.get("last_history_check", 0)
            if now - last_history >= HISTORY_CHECK_SECONDS:
                try:
                    process_history()
                    state["last_history_check"] = now
                except Exception as e:
                    print(f"Критична помилка історії: {e}")

            # Збереження загального стану
            save_state(state)

        except Exception as e:
            print(f"Загальна помилка в циклі: {e}")

        # Пауза між ітераціями циклу (наприклад, 60 секунд перед наступною перевіркою)
        print("Цикл завершено. Очікування наступної перевірки...")
        time.sleep(60)
